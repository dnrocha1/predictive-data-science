...
```{r}
#O TANTO DE DESPESA QUE NÃO É ATRIBUIDA A UM PARTIDO DE FORMA CLARA
dados %>% 
  filter(sgPartido %>% is.na())
```
...
top_despesas_partido <- dados %>% 
  ...
  rename(maiores_despesas = num_ocorrencias) %>% 
  na.omit()
...
#selecionando as despesas dos partidos escolhidos
